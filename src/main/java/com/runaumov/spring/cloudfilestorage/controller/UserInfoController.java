...
public class UserInfoController {
    ...
    @GetMapping("/me")
    public UserEntityResponseDto getCurrentUser(@AuthenticationPrincipal UserEntityDetails userDetails) {
        if (userDetails == null) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "User nit authenticated");
        }
        return new UserEntityResponseDto(userDetails.getUsername());
    }
}
